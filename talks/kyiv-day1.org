...
#+BEAMER: \pause
- Laws describe properties of these operations and connection between operations
  in one type class.
...
#+BEAMER: \pause
4. Task 2
   1. Create =Semigroup= typeclass manually in
      =wax.typeclass.semigroup.manual.typeclass=
   2. Implement missing =Semigroup= instances in
      =wax.typeclass.semigroup.manual.implicits=
   3. Run =wax.typeclass.semigroup.laws.manual.SemigroupSpec=
...
#+BEAMER: \pause
Laws give us *properties*. Which we use to get a solution that works for
everything that is ~Monoid~.

#+BEAMER: \pause
Monoids are everywhere around us. We deal with them every day, without even
noticing it. Did you expect us to solve Fibonacci using ~Monoid~?
...
#+BEAMER: \pause
- Just think about it, =mapReduce= is possible thanks to =Monoid= and its
  /laws/.
