...
\section*{Intro}
\label{sec:org91897bb}

\begin{frame}[label={sec:orgf969b37}]{About the Forkshop}
\begin{itemize}
...
\begin{frame}[label={sec:org047c330},fragile]{Whys}
 \begin{itemize}
...
\section*{Today}
\label{sec:orga5355e3}

\begin{frame}[label={sec:orgb469304}]{Agenda}
\begin{itemize}
\item Type classes
\item Semigroups
\item Monoids
...
\label{sec:orgbc7fb66}

\begin{frame}[label={sec:org0a510aa}]{Application definition}
\begin{itemize}
...
\begin{frame}[label={sec:orgf64599c},fragile]{Meet the hero}
 \begin{minted}[]{scala}
...
\begin{frame}[label={sec:org33d7832},fragile]{Every hero needs a monster}
 \begin{minted}[]{scala}
...
\begin{frame}[label={sec:org54adc6e},fragile]{Game}
 \begin{columns}
...
\begin{frame}[label={sec:org33bfa2f},fragile]{Introducing abstractions}
 \begin{columns}
...
\begin{frame}[label={sec:org9bf9fb4},fragile]{Game with trait}
 \begin{columns}
\begin{column}[t]{0.5\columnwidth}
\begin{minted}[]{scala}
...
\begin{frame}[label={sec:orgee26bc5},fragile]{Here comes the cockatrice}
 \begin{minted}[]{scala}
...
\begin{frame}[label={sec:org3562591},fragile]{Shawarma to the rescue}
 \begin{columns}
...
\begin{frame}[label={sec:org75d31f6}]{Calm down and reevaluate our goal}
\begin{itemize}
...
\begin{frame}[label={sec:org5bbd9ff},fragile]{\texttt{trait} + wrapper: abstraction}
...
\begin{frame}[label={sec:orgc7bd6d7},fragile]{\texttt{trait} + wrapper: composition}
...
\begin{frame}[label={sec:org34d564f},fragile]{\texttt{trait} + wrapper: extensibility}
...
\begin{frame}[label={sec:org3cd6344}]{You know where it’s going to, right?}
\pause
...
\begin{frame}[label={sec:orgf5b4370},fragile]{Dividing data and behaviour}
 \begin{columns}
\begin{column}[t]{0.5\columnwidth}
\begin{minted}[]{scala}
trait Introducible {
...
\begin{frame}[label={sec:orgea9cf47},fragile]{Usage}
 \begin{minted}[]{scala}
// Define new trait
trait Introducible[A] {
  def introduce(a: A): String
...
\begin{frame}[label={sec:org5f1dc28},fragile]{External types? Pff\ldots{}}
 \begin{minted}[]{scala}
import io.proprietary.monsters.cockatrice._
...
\begin{frame}[label={sec:org210cace}]{But passing implementation around is\ldots{}}
\begin{center}
...
\begin{frame}[label={sec:org1f2287a},fragile]{So implicits :(}
 \begin{columns}
\begin{column}[t]{0.5\columnwidth}
\begin{minted}[]{scala}
object Hero {
...
\begin{frame}[label={sec:orgf67a97a},fragile]{Summoning the summoner}
 \begin{columns}
\begin{column}[t]{0.5\columnwidth}
\begin{minted}[]{scala}
trait Introducible[A] {
...
\begin{frame}[label={sec:org6d4170f}]{What have we done?}
...
\begin{frame}[label={sec:org1a2c2b2},fragile]{Type classes: abstraction}
 Abstraction holds. Proof is the \texttt{introduce} function itself.
...
\begin{frame}[label={sec:orgfd88c61},fragile]{Type classes: composition}
...
\begin{frame}[label={sec:orgedf5fda}]{Type classes: extensibility}
...
\begin{frame}[label={sec:org876c464}]{Type classes: final thoughts}
\begin{enumerate}
...
\section*{Semigroup}
\label{sec:org2971d78}

\begin{frame}[label={sec:org3345d82}]{Time for a quiz!}
\begin{columns}
\begin{column}[t]{0.5\columnwidth}
...
\begin{frame}[label={sec:orgb178a5f}]{Associativity}
\begin{enumerate}
...
\begin{frame}[label={sec:org077f176}]{Semigroup}
...
\begin{frame}[label={sec:org44a66ad},fragile]{But it’s not that scary}
 \begin{columns}
...
\begin{frame}[label={sec:org033e862}]{Important!}
...
\begin{frame}[label={sec:org495ce77},fragile]{What is law?}
 \begin{itemize}
...
\begin{frame}[label={sec:org839e1d7},fragile]{Instance example}
 \begin{minted}[]{scala}
...
\begin{frame}[label={sec:org3b3c8d9},fragile]{Checking laws - \sout{pen and paper} in comments}
 \begin{minted}[]{scala}
package object implicits {
  implicit val stringSemigroup: Semigroup[String] = new Semigroup[String] {
    override def combine(x: String, y: String): String = x + y
...
\begin{frame}[label={sec:org8d81eae}]{You're programmer after all}
\begin{center}
\includegraphics[height=7cm]{images/you-re-programmer.jpg}
\end{center}
\end{frame}

\begin{frame}[label={sec:org58e846e},fragile]{Question on the interview: property based testing}
 \begin{minted}[]{scala}
...
\begin{frame}[label={sec:org4ef3121},fragile]{More than one valid instance}
 \begin{minted}[]{scala}
package object implicits {
  implicit val stringSemigroup: Semigroup[String] = new Semigroup[String] {
...
\begin{frame}[label={sec:orgb39af39}]{More examples}
\begin{itemize}
...
\begin{frame}[label={sec:org1b5d804}]{Contra-examples}
\begin{itemize}
...
\begin{frame}[label={sec:orgbc4db56},fragile]{Coding time}
 \begin{enumerate}
...
\section*{Monoid}
\label{sec:org1ec3515}

\begin{frame}[label={sec:org1838a2a}]{Monoid}
\begin{itemize}
...
\begin{frame}[label={sec:org56342b8}]{Monoid}
...
\begin{frame}[label={sec:org8ace583}]{Monoid}
Back to fancy words.
...
\begin{frame}[label={sec:org4615870},fragile]{Again, it's not that scary}
 \begin{minted}[]{scala}
package object typeclass {
...
\begin{frame}[label={sec:org507e33c},fragile]{Examples}
 \begin{itemize}
...
\begin{frame}[label={sec:org291e23e}]{The most important question}
\pause
...
\section*{Fibonacci}
\label{sec:orgb25b731}

\begin{frame}[label={sec:org4e410bc}]{The Fibonacci numbers}
\pause
...
\begin{frame}[label={sec:org3979fe7},fragile]{Solution}
 \begin{columns}
\begin{column}[t]{0.5\columnwidth}
...
\begin{frame}[label={sec:org7134e96},fragile]{Two folds (mod)}
 \begin{itemize}
...
\begin{frame}[label={sec:org96016ed},fragile]{Two folds}
 \begin{itemize}
\item \texttt{def foldl[A, B](xs: Seq[A])(z: B)(op: B => A => B): B}
...
\begin{frame}[label={sec:org71dff97},fragile]{Two folds}
 \begin{itemize}
\item \texttt{def foldl[A, B](xs: Seq[A])(z: B)(op: B => A => B): B}
\item \texttt{def foldr[A, B](xs: Seq[A])(z: B)(op: A => B => B): B}
...
\begin{frame}[label={sec:org11abc1b}]{Two folds}
\begin{columns}
\begin{column}[t]{0.5\columnwidth}
\begin{block}{foldl}
$$+ : B \rightarrow A \rightarrow B$$
...
\begin{frame}[label={sec:org8e0183c},fragile]{What Monoid gives us}
 \begin{itemize}
...
\begin{frame}[label={sec:orga14a0cd}]{Power in terms of Monoid}
...
\begin{frame}[label={sec:org57cfeec}]{Power in terms of Monoid}
\begin{center}
...
\begin{frame}[label={sec:org6f7f558}]{Power in terms of Monoid}
\begin{center}
...
\begin{frame}[label={sec:org2777fa4},fragile]{Power in terms of Monoid}
...
\begin{frame}[label={sec:orga2040cf}]{Back to Fibonacci}
...
\begin{frame}[label={sec:orgb125da7}]{Back to Fibonacci}
\begin{equation*}
  \begin{pmatrix}
    F_{n+1} & F_n \\
    F_n & F_{n-1}
...
\begin{frame}[label={sec:orgb6646a3},fragile]{Coding time}
 \begin{itemize}
...
\begin{frame}[label={sec:org1e5f0e1}]{Profiling results}
\begin{center}
\begin{tabular}{rrr}
N & Matrix & Tailrec\\
\hline
...
\begin{frame}[label={sec:org07c690c},fragile]{Outcome}
 \begin{itemize}
...
\section*{Books}
\label{sec:orge17b161}

\begin{frame}[label={sec:org0a37aaf}]{Folds with Monoids}
\begin{itemize}
...
\begin{frame}[label={sec:org4452874}]{Folds with Monoids}
\begin{center}
...
\begin{frame}[label={sec:orgbefb736},fragile]{MapReduce}
 \begin{itemize}
...
\begin{frame}[label={sec:org72afba5},fragile]{Coding time}
 \begin{itemize}
...
\begin{frame}[label={sec:org48ae952},fragile]{Benchmarks}
 \begin{verbatim}
Par
duration = 65633 ms
...
\begin{frame}[label={sec:org5430055},fragile]{Outcome}
 \begin{itemize}
...
\begin{frame}[label={sec:orgafb7531},fragile]{Homework}
...
\section*{Logger}
\label{sec:orge8f853e}

\begin{frame}[label={sec:orgf9965ac}]{Many monoids}
...
\begin{frame}[label={sec:orgbc46c66},fragile]{Let's start with some wrappers (pun intended)}
 \begin{itemize}
...
\begin{frame}[label={sec:org5981dcb},fragile]{Wrappers of monoids are monoids}
 \begin{itemize}
...
\begin{frame}[label={sec:org954df40},fragile]{Logger}
 \begin{itemize}
...
\begin{frame}[label={sec:org7713155},fragile]{Logger}
 \begin{minted}[]{scala}
...
\begin{frame}[label={sec:orgf78a3b6},fragile]{Logger}
 \begin{itemize}
...
\begin{frame}[label={sec:org42e3b10},fragile]{Bonus questions}
 \begin{itemize}
...
\begin{frame}[label={sec:orge19acb4},fragile]{Outcome}
 \begin{itemize}
...
\section*{Recap}
\label{sec:orgc7a5b87}

\begin{frame}[label={sec:orgecfd2bd},fragile]{Recap (recup?)}
 \begin{itemize}
...
\begin{frame}[label={sec:org2ee00b4}]{Questions?}
\centerline{\huge $\epsilon \rho \omega \tau \eta \sigma \eta \varsigma$?}
\end{frame}

\begin{frame}[label={sec:org2490d29}]{Thank you very much!}
