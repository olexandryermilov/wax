...
\section*{Intro}
\label{sec:orgd5cba70}

\begin{frame}[label={sec:org244bd48}]{About us}
\begin{block}{Valik}
...
\begin{frame}[label={sec:orgc65c51a}]{About the Forkshop}
\begin{itemize}
...
\begin{frame}[label={sec:orgef87711},fragile]{Whys}
 \begin{itemize}
...
\section*{Today}
\label{sec:org8a26a82}

\begin{frame}[label={sec:org5f03d90}]{Agenda}
\begin{itemize}
\item Type classes
\item Semigroups
\item Monoids
...
\label{sec:orgcb903b0}

\begin{frame}[label={sec:org2560d5f}]{Application definition}
\begin{itemize}
...
\begin{frame}[label={sec:org89da7cc},fragile]{Meet the hero}
 \begin{minted}[]{scala}
...
\begin{frame}[label={sec:org9aec378},fragile]{Every hero needs a monster}
 \begin{minted}[]{scala}
...
\begin{frame}[label={sec:org6b09348},fragile]{Game}
 \begin{columns}
...
\begin{frame}[label={sec:org05b51b4},fragile]{Introducing abstractions}
 \begin{columns}
...
\begin{frame}[label={sec:orgb2eed71},fragile]{Game with trait}
 \begin{itemize}
...
\begin{frame}[label={sec:orge4d610b},fragile]{Here comes the cockatrice}
 \begin{minted}[]{scala}
...
\begin{frame}[label={sec:orgb632789},fragile]{Shawarma to the rescue}
 \begin{columns}
...
\begin{frame}[label={sec:orge02c984}]{Calm down and reevaluate our goal}
\begin{itemize}
...
\begin{frame}[label={sec:orgeabd318},fragile]{\texttt{trait} + wrapper: abstraction}
...
\begin{frame}[label={sec:org432d822},fragile]{\texttt{trait} + wrapper: composition}
...
\begin{frame}[label={sec:org00f6000},fragile]{\texttt{trait} + wrapper: extensibility}
...
\begin{frame}[label={sec:org90d2013}]{You know where it’s going to, right?}
\pause
...
\begin{frame}[label={sec:org5e36158},fragile]{Dividing data and behaviour}
 \begin{columns}
\begin{column}[t]{0.5\columnwidth}
\begin{minted}[]{scala}
trait Introducible {
...
\begin{frame}[label={sec:orge00a73f},fragile]{Usage}
 \begin{minted}[]{scala}
// Define new trait
trait Introducible[A] {
  def introduce(a: A): String
...
\begin{frame}[label={sec:org03f7821},fragile]{External types? Pff\ldots{}}
 \begin{minted}[]{scala}
import io.proprietary.monsters.cockatrice._
...
\begin{frame}[label={sec:orga8fcb9b}]{But passing implementation around is\ldots{}}
\begin{center}
...
\begin{frame}[label={sec:org77e9cb1},fragile]{So implicits :(}
 \begin{columns}
\begin{column}[t]{0.5\columnwidth}
\begin{minted}[]{scala}
object Hero {
...
\begin{frame}[label={sec:orge4bc726},fragile]{Summoning the summoner}
 \begin{columns}
\begin{column}[t]{0.5\columnwidth}
\begin{minted}[]{scala}
trait Introducible[A] {
...
\begin{frame}[label={sec:org567d924}]{What have we done?}
...
\begin{frame}[label={sec:orgb8b4256},fragile]{Type classes: abstraction}
 Abstraction holds. Proof is the \texttt{introduce} function itself.
...
\begin{frame}[label={sec:org1bf5cfc},fragile]{Type classes: composition}
...
\begin{frame}[label={sec:org1df5fe1}]{Type classes: extensibility}
...
\begin{frame}[label={sec:org40e1536}]{Type classes: final thoughts}
\begin{enumerate}
...
\section*{Semigroup}
\label{sec:org389d7b9}

\begin{frame}[label={sec:orgb3e2770}]{Time for a quiz!}
\begin{columns}
\begin{column}[t]{0.5\columnwidth}
...
\begin{frame}[label={sec:org73ef4e5}]{Associativity}
...
\begin{frame}[label={sec:org2d584e5}]{Semigroup}
...
\begin{frame}[label={sec:orgc3c58a3},fragile]{But it’s not that scary}
 \begin{columns}
...
\begin{frame}[label={sec:orga55c998}]{Important!}
...
\begin{frame}[label={sec:org3f52c2e},fragile]{What is law?}
 \begin{itemize}
...
\begin{frame}[label={sec:org3dbbe83},fragile]{Instance example}
 \begin{minted}[]{scala}
...
\begin{frame}[label={sec:org587651d},fragile]{Checking laws - \sout{pen and paper} in comments}
 \begin{minted}[]{scala}
package object implicits {
  implicit val stringSemigroup: Semigroup[String] = new Semigroup[String] {
    override def combine(x: String, y: String): String = x + y
...
\begin{frame}[label={sec:orga1acd8e}]{You're programmer after all}
\begin{center}
\includegraphics[height=7cm]{images/you-re-programmer.jpg}
\end{center}
\end{frame}

\begin{frame}[label={sec:orgb91ebe4},fragile]{Question on the interview: property based testing}
 \begin{minted}[]{scala}
...
\begin{frame}[label={sec:org162fae7},fragile]{More than one valid instance}
 \begin{minted}[]{scala}
package object implicits {
  implicit val stringSemigroup: Semigroup[String] = new Semigroup[String] {
...
\begin{frame}[label={sec:org97a0834}]{More examples}
\begin{itemize}
...
\begin{frame}[label={sec:orgacb4fc1}]{Contra-examples}
\begin{itemize}
...
\begin{frame}[label={sec:org972f722},fragile]{Coding time}
 \begin{enumerate}
...
\section*{Monoid}
\label{sec:org3fa7035}

\begin{frame}[label={sec:orgc406116}]{Monoid}
\begin{itemize}
...
\begin{frame}[label={sec:orgeb27dab}]{Monoid}
...
\begin{frame}[label={sec:org49e3bde}]{Monoid}
Back to fancy words.
...
\begin{frame}[label={sec:org76ea8c5},fragile]{Again, it's not that scary}
 \begin{minted}[]{scala}
package object typeclass {
...
\begin{frame}[label={sec:orgacc157d},fragile]{Examples}
 \begin{itemize}
...
\begin{frame}[label={sec:orgdd622b1}]{The most important question}
\pause
...
\section*{Fibonacci}
\label{sec:orgffe697d}

\begin{frame}[label={sec:orgda406b9}]{The Fibonacci numbers}
\pause
...
\begin{frame}[label={sec:orgedfea45},fragile]{Solution}
 \begin{columns}
\begin{column}[t]{0.5\columnwidth}
...
\begin{frame}[label={sec:org6b5692b},fragile]{Two folds}
 \begin{itemize}
...
\begin{frame}[label={sec:org5e85c39},fragile]{Two folds}
 \begin{itemize}
\item \texttt{def foldl[A, B](xs: Seq[A])(z: B)(op: B => A => B): B}
...
\begin{frame}[label={sec:orga762ed2},fragile]{Two folds}
 \begin{itemize}
\item \texttt{def foldl[A, B](xs: Seq[A])(z: B)(op: B => A => B): B}
\item \texttt{def foldr[A, B](xs: Seq[A])(z: B)(op: A => B => B): B}
...
\begin{frame}[label={sec:orgd9ac1c3}]{Two folds}
\begin{columns}
\begin{column}[t]{0.5\columnwidth}
\begin{block}{foldl}
\begin{center}
...
\begin{frame}[label={sec:org09316a0},fragile]{What Monoid gives us}
 \begin{itemize}
...
\begin{frame}[label={sec:org8dd910d}]{Power in terms of Monoid}
...
\begin{frame}[label={sec:orgdc32ff6}]{Power in terms of Monoid}
\begin{center}
...
\begin{frame}[label={sec:org194f843}]{Power in terms of Monoid}
\begin{center}
...
\begin{frame}[label={sec:org7d9455e},fragile]{Power in terms of Monoid}
...
\begin{frame}[label={sec:orgefdac41}]{Back to Fibonacci}
...
\begin{frame}[label={sec:org49482a8}]{Back to Fibonacci}
\begin{equation*}
  \begin{pmatrix}
    F_{n+1} & F_n \\
    F_n & F_{n-1}
...
\begin{frame}[label={sec:orgc6ae634},fragile]{Coding time}
 \begin{itemize}
...
\begin{frame}[label={sec:org14ae261}]{Profiling results}
\begin{center}
\begin{tabular}{rrr}
N & Matrix & Tailrec\\
\hline
...
\begin{frame}[label={sec:orgfd3cc0b},fragile]{Outcome}
 \begin{itemize}
...
\section*{Books}
\label{sec:orgd4af13f}

\begin{frame}[label={sec:org7f82b86}]{Folds with Monoids}
\begin{itemize}
...
\begin{frame}[label={sec:orge7d9064}]{Folds with Monoids}
\begin{center}
...
\begin{frame}[label={sec:org3df739a},fragile]{MapReduce}
 \begin{itemize}
...
\begin{frame}[label={sec:org553d940},fragile]{Coding time}
 \begin{itemize}
...
\begin{frame}[label={sec:org1365e01},fragile]{Benchmarks}
 \begin{verbatim}
Par
duration = 65633 ms
...
\begin{frame}[label={sec:orga1da42b},fragile]{Outcome}
 \begin{itemize}
...
\begin{frame}[label={sec:org0b7996a},fragile]{Homework}
...
\section*{Logger}
\label{sec:org2842889}

\begin{frame}[label={sec:org544c711}]{Many monoids}
...
\begin{frame}[label={sec:org23b81ff},fragile]{Let's start with some wrappers (pun intended)}
 \begin{itemize}
...
\begin{frame}[label={sec:org4db65b7},fragile]{Wrappers of monoids are monoids}
 \begin{itemize}
...
\begin{frame}[label={sec:orge88a3b9},fragile]{Logger}
 \begin{itemize}
...
\begin{frame}[label={sec:org444a75e},fragile]{Logger}
 \begin{minted}[]{scala}
...
\begin{frame}[label={sec:org3bf411c},fragile]{Logger}
 \begin{itemize}
...
\begin{frame}[label={sec:orge8961c3},fragile]{Bonus questions}
 \begin{itemize}
...
\begin{frame}[label={sec:org7be6310},fragile]{Outcome}
 \begin{itemize}
...
\section*{Recap}
\label{sec:org8689433}

\begin{frame}[label={sec:org6032d71},fragile]{Recap (recup?)}
 \begin{itemize}
...
\begin{frame}[label={sec:org52d5b66}]{Questions?}
\centerline{\huge $\epsilon \rho \omega \tau \eta \sigma \eta$?}
\end{frame}

\begin{frame}[label={sec:orgb8c8e6a}]{Thank you very much!}
