...
\section*{Intro}
\label{sec:org3c38057}

\begin{frame}[label={sec:org4891375}]{About us}
\begin{block}{Valik}
...
\begin{frame}[label={sec:orgced9724}]{About the Forkshop}
\begin{itemize}
...
\begin{frame}[label={sec:org398dfdb},fragile]{Whys}
 \begin{itemize}
...
\section*{Today}
\label{sec:org3ef4e38}

\begin{frame}[label={sec:orgb36d036}]{Agenda}
\begin{itemize}
\item Type classes
\item Semigroups
\item Monoids
...
\label{sec:org9ffe88a}

\begin{frame}[label={sec:org79306d6}]{Type classes}
\begin{itemize}
\item Problem:
\begin{itemize}
...
\begin{frame}[label={sec:org9e5b844}]{Application definition}
\begin{itemize}
...
\begin{frame}[label={sec:orgdded28a},fragile]{Meet the hero}
 \begin{minted}[]{scala}
...
\begin{frame}[label={sec:org9e7d50f},fragile]{Every hero needs a monster}
 \begin{minted}[]{scala}
...
\begin{frame}[label={sec:org3c7e06e},fragile]{Game}
 \begin{columns}
...
\begin{frame}[label={sec:org2b14d53},fragile]{Be DRY, leap for disaster}
 \begin{columns}
\begin{column}[t]{0.46\columnwidth}
\begin{minted}[]{scala}
object Game extends App {
...
\begin{frame}[label={sec:org32022e8},fragile]{Introducing abstractions}
 \begin{columns}
...
\begin{frame}[label={sec:orgf66b45a},fragile]{Game with \texttt{trait}}
 \begin{columns}
\begin{column}[t]{0.5\columnwidth}
\begin{minted}[]{scala}
def introduce(phrase: String): Unit = {
...
\begin{frame}[label={sec:org014a41a},fragile]{Game with trait}
 \begin{itemize}
...
\begin{frame}[label={sec:org6d413cc},fragile]{Here comes the cockatrice}
 \begin{minted}[]{scala}
...
\begin{frame}[label={sec:org96f75a4},fragile]{Shawarma to the rescue}
 \begin{columns}
...
\begin{frame}[label={sec:org7c74224}]{Calm down and reevaluate our goal}
\begin{itemize}
...
\begin{frame}[label={sec:orgd454a1c},fragile]{\texttt{trait} + wrapper: abstraction}
...
\begin{frame}[label={sec:orgdcc3301},fragile]{\texttt{trait} + wrapper: composition}
...
\begin{frame}[label={sec:org764c518},fragile]{\texttt{trait} + wrapper: extensibility}
...
\begin{frame}[label={sec:org70873c5}]{You know where it’s going to, right?}
\pause
...
\begin{frame}[label={sec:org93e7e70},fragile]{Dividing data and behaviour}
 \begin{columns}
\begin{column}[t]{0.5\columnwidth}
\begin{minted}[]{scala}
trait Introducible {
...
\begin{frame}[label={sec:org1d256af},fragile]{Usage}
 \begin{minted}[]{scala}
// Define new trait
trait Introducible[A] {
  def introduce(a: A): String
...
\begin{frame}[label={sec:orgaa7ac15},fragile]{External types? Pff\ldots{}}
 \begin{minted}[]{scala}
import io.proprietary.monsters.cockatrice._
...
\begin{frame}[label={sec:org8db421d}]{But passing implementation around is\ldots{}}
\begin{center}
...
\begin{frame}[label={sec:orgadcbf6c},fragile]{So implicits :(}
 \begin{columns}
\begin{column}[t]{0.5\columnwidth}
\begin{minted}[]{scala}
object Hero {
...
\begin{frame}[label={sec:orgbe0e619},fragile]{Summoning the summoner}
 \begin{columns}
\begin{column}[t]{0.5\columnwidth}
\begin{minted}[]{scala}
trait Introducible[A] {
...
\begin{frame}[label={sec:org4e79bc3}]{What have we done?}
...
\begin{frame}[label={sec:org58c6058},fragile]{Type classes: abstraction}
 Abstraction holds. Proof is the \texttt{introduce} function itself.
...
\begin{frame}[label={sec:orgeb8442e},fragile]{Type classes: composition}
...
\begin{frame}[label={sec:org83067d9}]{Type classes: extensibility}
...
\begin{frame}[label={sec:org4f089b5}]{Type classes: final thoughts}
\begin{enumerate}
...
\section*{Transition}
\label{sec:org263b6be}

\begin{frame}[label={sec:org1532bfa}]{Warning}
...
\section*{Semigroup}
\label{sec:org600dd51}

\begin{frame}[label={sec:org5a797bd}]{Semigroup}
...
\begin{frame}[label={sec:orge03a2d2},fragile]{But it’s not that scary}
 \begin{columns}
...
\begin{frame}[label={sec:orgb731669}]{Important!}
...
\begin{frame}[label={sec:orga4c39c1},fragile]{What is law?}
 \begin{itemize}
...
\begin{frame}[label={sec:org2431f93},fragile]{Instance example}
 \begin{minted}[]{scala}
...
\begin{frame}[label={sec:orga1d6bc9},fragile]{Checking laws - \sout{pen and paper} in comments}
 \begin{minted}[]{scala}
package object implicits {
  implicit val stringSemigroup: Semigroup[String] = new Semigroup[String] {
    override def combine(x: String, y: String): String = x + y
...
\begin{frame}[label={sec:org8133399}]{You're programmer after all}
\begin{center}
\includegraphics[height=7cm]{images/you-re-programmer.jpg}
\end{center}
\end{frame}

\begin{frame}[label={sec:org9ac7c63},fragile]{Question on the interview: property based testing}
 \begin{minted}[]{scala}
...
\begin{frame}[label={sec:orgb21d6a7},fragile]{More than one valid instance}
 \begin{minted}[]{scala}
package object implicits {
  implicit val stringSemigroup: Semigroup[String] = new Semigroup[String] {
...
\begin{frame}[label={sec:org360be05}]{More examples}
\begin{itemize}
...
\begin{frame}[label={sec:org17186e6}]{Contra-examples}
\begin{itemize}
...
\begin{frame}[label={sec:orgc888073},fragile]{Coding time}
 \begin{enumerate}
...
\section*{Monoid}
\label{sec:org0bd140d}

\begin{frame}[label={sec:orgcb2f99b}]{Monoid}
...
\begin{frame}[label={sec:orgd5b6265},fragile]{Again, it's not that scary}
 \begin{minted}[]{scala}
package object typeclass {
...
\begin{frame}[label={sec:orge8fffd9},fragile]{Examples}
 \begin{itemize}
...
\label{sec:org8f0ef17}

\begin{frame}[label={sec:orgba00983}]{The most important question}
\pause
...
\section*{Fibonacci}
\label{sec:org55914a7}

\begin{frame}[label={sec:orgcfacdcc}]{The Fibonacci numbers}
\pause
...
\begin{frame}[label={sec:org7c10d01},fragile]{Solution}
 \begin{columns}
\begin{column}[t]{0.5\columnwidth}
...
\begin{frame}[label={sec:orgaaa6f71},fragile]{Two folds}
 \begin{itemize}
...
\begin{frame}[label={sec:org920de47},fragile]{Two folds}
 \begin{itemize}
\item \texttt{def foldl[A, B](xs: Seq[A])(z: B)(op: B => A => B): B}
...
\begin{frame}[label={sec:org8c68d71},fragile]{Two folds}
 \begin{itemize}
\item \texttt{def foldl[A, B](xs: Seq[A])(z: B)(op: B => A => B): B}
\item \texttt{def foldr[A, B](xs: Seq[A])(z: B)(op: A => B => B): B}
...
\begin{frame}[label={sec:orgc1ef7b9}]{Two folds}
\begin{columns}
\begin{column}[t]{0.5\columnwidth}
\begin{block}{foldl}
\begin{center}
...
\begin{frame}[label={sec:org09a6d66},fragile]{What Monoid gives us}
 \begin{itemize}
...
\begin{frame}[label={sec:org63263a2}]{Power in terms of Monoid}
\begin{itemize}
...
\begin{frame}[label={sec:org3dc0fd1}]{Power in terms of Monoid}
\begin{center}
...
\begin{frame}[label={sec:org2ab8493}]{Power in terms of Monoid}
\begin{center}
...
\begin{frame}[label={sec:org36a62f8},fragile]{Power in terms of Monoid}
...
\begin{frame}[label={sec:orgd8054ae}]{Back to Fibonacci}
...
\begin{frame}[label={sec:orgd92d426}]{Back to Fibonacci}
\begin{equation*}
  \begin{pmatrix}
    F_{n+1} & F_n \\
    F_n & F_{n-1}
...
\begin{frame}[label={sec:orgeefe9e8},fragile]{Coding time}
 \begin{itemize}
...
\begin{frame}[label={sec:org784eaf6}]{Profiling results}
\begin{center}
\begin{tabular}{rrr}
N & Matrix & Tailrec\\
\hline
...
\begin{frame}[label={sec:org165f2f9},fragile]{Outcome}
 Just think about it.
...
\section*{Books}
\label{sec:org2effd36}

\begin{frame}[label={sec:org17218e0}]{Folds with Monoids}
\begin{itemize}
...
\begin{frame}[label={sec:org6965b50}]{Folds with Monoids}
\begin{center}
...
\begin{frame}[label={sec:org26973f9},fragile]{MapReduce}
 \begin{itemize}
...
\begin{frame}[label={sec:org4127f47},fragile]{Coding time}
 \begin{itemize}
...
\section*{Logger}
\label{sec:org99ac60f}

\begin{frame}[label={sec:orgaa54a92},fragile]{Things to note}
 \begin{itemize}
...
\begin{frame}[label={sec:org9694f2c},fragile]{Let's start with some wrappers (pun intended)}
 \begin{itemize}
...
\begin{frame}[label={sec:org84bdb80},fragile]{Wrappers of monoids are monoids}
 \begin{itemize}
...
\begin{frame}[label={sec:orgc5f0c89},fragile]{Logger}
 \begin{itemize}
...
\begin{frame}[label={sec:org679e2d4},fragile]{Logger}
 \begin{itemize}
...
\section*{Recap}
\label{sec:org2b979e5}

\begin{frame}[label={sec:orgb48b1de},fragile]{Recap (recup?)}
 \begin{itemize}
...
\begin{frame}[label={sec:org6c98312}]{Questions?}
\centerline{\huge $\epsilon \rho \omega \tau \eta \sigma \eta$?}
\end{frame}

\begin{frame}[label={sec:org43fc4cf}]{Thank you very much!}
