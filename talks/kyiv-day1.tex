...
\section*{Intro}
\label{sec:org6b86877}

\begin{frame}[label={sec:org8615d2e}]{About us}
\begin{block}{Valik}
...
\begin{frame}[label={sec:org11e945a}]{About the Forkshop}
\begin{itemize}
...
\begin{frame}[label={sec:org82a9dc3},fragile]{Whys}
 \begin{itemize}
...
\section*{Today}
\label{sec:org74641bf}

\begin{frame}[label={sec:orgd9ba305}]{Agenda}
\begin{itemize}
\item Type classes
\item Semigroups
\item Monoids
...
\label{sec:orgcb9fce9}

\begin{frame}[label={sec:orgb39c94d}]{Application definition}
\begin{itemize}
...
\begin{frame}[label={sec:org277f8bc},fragile]{Meet the hero}
 \begin{minted}[]{scala}
...
\begin{frame}[label={sec:org67be284},fragile]{Every hero needs a monster}
 \begin{minted}[]{scala}
...
\begin{frame}[label={sec:org230e402},fragile]{Game}
 \begin{columns}
...
\begin{frame}[label={sec:orgb09e997},fragile]{Introducing abstractions}
 \begin{columns}
...
\begin{frame}[label={sec:orgf0718d3},fragile]{Game with trait}
 \begin{columns}
\begin{column}[t]{0.5\columnwidth}
\begin{minted}[]{scala}
...
\begin{frame}[label={sec:orgae7b77f},fragile]{Here comes the cockatrice}
 \begin{minted}[]{scala}
...
\begin{frame}[label={sec:orgbcbf92a},fragile]{Shawarma to the rescue}
 \begin{columns}
...
\begin{frame}[label={sec:org5851780}]{Calm down and reevaluate our goal}
\begin{itemize}
...
\begin{frame}[label={sec:org71626bd},fragile]{\texttt{trait} + wrapper: abstraction}
...
\begin{frame}[label={sec:org8bf35c9},fragile]{\texttt{trait} + wrapper: composition}
...
\begin{frame}[label={sec:org01484ca},fragile]{\texttt{trait} + wrapper: extensibility}
...
\begin{frame}[label={sec:org0e590e8}]{You know where it’s going to, right?}
\pause
...
\begin{frame}[label={sec:orgcb712f2},fragile]{Dividing data and behaviour}
 \begin{columns}
\begin{column}[t]{0.5\columnwidth}
\begin{minted}[]{scala}
trait Introducible {
...
\begin{frame}[label={sec:orga36b1b7},fragile]{Usage}
 \begin{minted}[]{scala}
// Define new trait
trait Introducible[A] {
  def introduce(a: A): String
...
\begin{frame}[label={sec:org2e69a6a},fragile]{External types? Pff\ldots{}}
 \begin{minted}[]{scala}
import io.proprietary.monsters.cockatrice._
...
\begin{frame}[label={sec:orgd5c6a57}]{But passing implementation around is\ldots{}}
\begin{center}
...
\begin{frame}[label={sec:orga8be68e},fragile]{So implicits :(}
 \begin{columns}
\begin{column}[t]{0.5\columnwidth}
\begin{minted}[]{scala}
object Hero {
...
\begin{frame}[label={sec:org24ba208},fragile]{Summoning the summoner}
 \begin{columns}
\begin{column}[t]{0.5\columnwidth}
\begin{minted}[]{scala}
trait Introducible[A] {
...
\begin{frame}[label={sec:orgfc4d26f}]{What have we done?}
...
\begin{frame}[label={sec:org1cfd51e},fragile]{Type classes: abstraction}
 Abstraction holds. Proof is the \texttt{introduce} function itself.
...
\begin{frame}[label={sec:orge823f41},fragile]{Type classes: composition}
...
\begin{frame}[label={sec:org42c9340}]{Type classes: extensibility}
...
\begin{frame}[label={sec:orga3d1821}]{Type classes: final thoughts}
\begin{enumerate}
...
\section*{Semigroup}
\label{sec:org1dea600}

\begin{frame}[label={sec:orge968f62}]{Time for a quiz!}
\begin{columns}
\begin{column}[t]{0.5\columnwidth}
...
\begin{frame}[label={sec:org1475bfe}]{Associativity}
\begin{enumerate}
...
\begin{frame}[label={sec:org4408b59}]{Semigroup}
...
\begin{frame}[label={sec:org57b676d},fragile]{But it’s not that scary}
 \begin{columns}
...
\begin{frame}[label={sec:orgabf132b}]{Important!}
...
\begin{frame}[label={sec:org4746f2d},fragile]{What is law?}
 \begin{itemize}
...
\begin{frame}[label={sec:org42a3a70},fragile]{Instance example}
 \begin{minted}[]{scala}
...
\begin{frame}[label={sec:orgdf5c83c},fragile]{Checking laws - \sout{pen and paper} in comments}
 \begin{minted}[]{scala}
package object implicits {
  implicit val stringSemigroup: Semigroup[String] = new Semigroup[String] {
    override def combine(x: String, y: String): String = x + y
...
\begin{frame}[label={sec:orgf3bc406}]{You're programmer after all}
\begin{center}
\includegraphics[height=7cm]{images/you-re-programmer.jpg}
\end{center}
\end{frame}

\begin{frame}[label={sec:orgb01e1e5},fragile]{Question on the interview: property based testing}
 \begin{minted}[]{scala}
...
\begin{frame}[label={sec:org16b74b9},fragile]{More than one valid instance}
 \begin{minted}[]{scala}
package object implicits {
  implicit val stringSemigroup: Semigroup[String] = new Semigroup[String] {
...
\begin{frame}[label={sec:orgf905001}]{More examples}
\begin{itemize}
...
\begin{frame}[label={sec:org2793051}]{Contra-examples}
\begin{itemize}
...
\begin{frame}[label={sec:orgd64ede1},fragile]{Coding time}
 \begin{enumerate}
...
\section*{Monoid}
\label{sec:org31c49f1}

\begin{frame}[label={sec:org0a326d2}]{Monoid}
\begin{itemize}
...
\begin{frame}[label={sec:org4d5be3f}]{Monoid}
...
\begin{frame}[label={sec:org445ed05}]{Monoid}
Back to fancy words.
...
\begin{frame}[label={sec:org6396db6},fragile]{Again, it's not that scary}
 \begin{minted}[]{scala}
package object typeclass {
...
\begin{frame}[label={sec:orgf2597ec},fragile]{Examples}
 \begin{itemize}
...
\begin{frame}[label={sec:org46bacb8}]{The most important question}
\pause
...
\section*{Fibonacci}
\label{sec:orgb49faf5}

\begin{frame}[label={sec:orgb73a215}]{The Fibonacci numbers}
\pause
...
\begin{frame}[label={sec:org7092db4},fragile]{Solution}
 \begin{columns}
\begin{column}[t]{0.5\columnwidth}
...
\begin{frame}[label={sec:org8b9a680},fragile]{Two folds (mod)}
 \begin{itemize}
...
\begin{center}
\includegraphics[height=5cm]{diag/foldl-tree.png}
\end{center}
\end{frame}

\begin{frame}[label={sec:org2e94498},fragile]{Two folds}
 \begin{itemize}
\item \texttt{def foldl[A, B](xs: Seq[A])(z: B)(op: B => A => B): B}
...
\begin{center}
\includegraphics[height=5cm]{diag/foldr-tree.png}
\end{center}
\end{frame}

\begin{frame}[label={sec:org772679d},fragile]{Two folds}
 \begin{itemize}
\item \texttt{def foldl[A, B](xs: Seq[A])(z: B)(op: B => A => B): B}
\item \texttt{def foldr[A, B](xs: Seq[A])(z: B)(op: A => B => B): B}
...
\begin{frame}[label={sec:org1e1d1f7}]{Two folds}
\begin{columns}
\begin{column}[t]{0.5\columnwidth}
\begin{block}{foldl}
$$+ : B \rightarrow A \rightarrow B$$
...
\begin{frame}[label={sec:org464ceba},fragile]{What Monoid gives us}
 \begin{itemize}
...
\begin{frame}[label={sec:org7a94fe8}]{Power in terms of Monoid}
...
\begin{frame}[label={sec:orgc51a6dd}]{Power in terms of Monoid}
\begin{center}
...
\begin{frame}[label={sec:org42c71fc}]{Power in terms of Monoid}
\begin{center}
...
\begin{frame}[label={sec:org022564f},fragile]{Power in terms of Monoid}
...
\begin{frame}[label={sec:org1684920}]{Back to Fibonacci}
...
\begin{frame}[label={sec:org096c9b2}]{Back to Fibonacci}
\begin{equation*}
  \begin{pmatrix}
    F_{n+1} & F_n \\
    F_n & F_{n-1}
...
\begin{frame}[label={sec:orgb63218f},fragile]{Coding time}
 \begin{itemize}
...
\begin{frame}[label={sec:orgfaa6ddc}]{Profiling results}
\begin{center}
\begin{tabular}{rrr}
N & Matrix & Tailrec\\
\hline
...
\begin{frame}[label={sec:orgc408935},fragile]{Outcome}
 \begin{itemize}
...
\section*{Books}
\label{sec:org956fa09}

\begin{frame}[label={sec:org4c5d3a1}]{Folds with Monoids}
\begin{itemize}
...
\begin{frame}[label={sec:org6d5ece1}]{Folds with Monoids}
\begin{center}
...
\begin{frame}[label={sec:orgfa1320e},fragile]{MapReduce}
 \begin{itemize}
...
\begin{frame}[label={sec:org2e11e54},fragile]{Coding time}
 \begin{itemize}
...
\begin{frame}[label={sec:orga615e06},fragile]{Benchmarks}
 \begin{verbatim}
Par
duration = 65633 ms
...
\begin{frame}[label={sec:orgc43a7de},fragile]{Outcome}
 \begin{itemize}
...
\begin{frame}[label={sec:orgbf5f088},fragile]{Homework}
...
\section*{Logger}
\label{sec:org6180a23}

\begin{frame}[label={sec:org8b5f3d8}]{Many monoids}
...
\begin{frame}[label={sec:org7fe8e2e},fragile]{Let's start with some wrappers (pun intended)}
 \begin{itemize}
...
\begin{frame}[label={sec:orgfb88572},fragile]{Wrappers of monoids are monoids}
 \begin{itemize}
...
\begin{frame}[label={sec:org620f64f},fragile]{Logger}
 \begin{itemize}
...
\begin{frame}[label={sec:orgabbdd6e},fragile]{Logger}
 \begin{minted}[]{scala}
...
\begin{frame}[label={sec:orgc31949f},fragile]{Logger}
 \begin{itemize}
...
\begin{frame}[label={sec:orgcee27df},fragile]{Bonus questions}
 \begin{itemize}
...
\begin{frame}[label={sec:orga0e8deb},fragile]{Outcome}
 \begin{itemize}
...
\section*{Recap}
\label{sec:orgc140090}

\begin{frame}[label={sec:orgc574581},fragile]{Recap (recup?)}
 \begin{itemize}
...
\begin{frame}[label={sec:orga7e4f89}]{Questions?}
\centerline{\huge $\epsilon \rho \omega \tau \eta \sigma \eta \varsigma$?}
\end{frame}

\begin{frame}[label={sec:org8b95fae}]{Thank you very much!}
