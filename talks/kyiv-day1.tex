...
\section*{Intro}
\label{sec:org62506f6}

\begin{frame}[label={sec:org18d6ccb}]{About us}
\begin{block}{Valik}
...
\begin{frame}[label={sec:org41aad95}]{About the Forkshop}
\begin{itemize}
...
\begin{frame}[label={sec:orgdc58f8f},fragile]{Whys}
 \begin{itemize}
...
\section*{Today}
\label{sec:org0e2cfcf}

\begin{frame}[label={sec:orgc5c3b88}]{Agenda}
\begin{itemize}
\item Type classes
\item Semigroups
\item Monoids
...
\label{sec:orgd541788}

\begin{frame}[label={sec:org0f6fa24}]{Type classes}
\begin{itemize}
\item <1-> Problem:
\begin{itemize}
...
\begin{frame}[label={sec:orgec89799}]{Application definition}
\begin{itemize}
...
\begin{frame}[label={sec:org1470650},fragile]{Meet the hero}
 \begin{minted}[]{scala}
...
\begin{frame}[label={sec:org8e658d1},fragile]{Every hero needs a monster}
 \begin{minted}[]{scala}
...
\begin{frame}[label={sec:org6c3fd0d},fragile]{Game}
 \begin{columns}
...
\begin{frame}[label={sec:org1091137},fragile]{Be DRY, leap for disaster}
 \begin{columns}
\begin{column}[t]{0.46\columnwidth}
\begin{minted}[]{scala}
object Game extends App {
...
\begin{frame}[label={sec:org7f493a0},fragile]{Introducing abstractions}
 \begin{columns}
...
\begin{frame}[label={sec:orgb075785},fragile]{Game with \texttt{trait}}
 \begin{columns}
\begin{column}[t]{0.5\columnwidth}
\begin{minted}[]{scala}
def introduce(phrase: String): Unit = {
...
\begin{frame}[label={sec:orga4b5dfe},fragile]{Game with trait}
 \begin{itemize}
...
\begin{frame}[label={sec:org5fa60f3},fragile]{Here comes the cockatrice}
 \begin{minted}[]{scala}
...
\begin{frame}[label={sec:org3e73eb4},fragile]{Shawarma to the rescue}
 \begin{columns}
...
\begin{frame}[label={sec:orgb6e94c7}]{Calm down and reevaluate our goal}
\begin{itemize}
...
\begin{frame}[label={sec:org9910090},fragile]{\texttt{trait} + wrapper: abstraction}
...
\begin{frame}[label={sec:orgca63bd1},fragile]{\texttt{trait} + wrapper: composition}
...
\begin{frame}[label={sec:orga2adf92},fragile]{\texttt{trait} + wrapper: extensibility}
...
\begin{frame}[label={sec:org80c41b5}]{You know where it’s going to, right?}
\pause
...
\begin{frame}[label={sec:org0b13b94},fragile]{Dividing data and behaviour}
 \begin{columns}
\begin{column}[t]{0.5\columnwidth}
\begin{minted}[]{scala}
trait Introducible {
...
\begin{frame}[label={sec:orgab49009},fragile]{Usage}
 \begin{minted}[]{scala}
// Define new trait
trait Introducible[A] {
  def introduce(a: A): String
...
\begin{frame}[label={sec:org33113ad},fragile]{External types? Pff\ldots{}}
 \begin{minted}[]{scala}
import io.proprietary.monsters.cockatrice._
...
\begin{frame}[label={sec:org498b149}]{But passing implementation around is\ldots{}}
\begin{center}
...
\begin{frame}[label={sec:orgc6e1a78},fragile]{So implicits :(}
 \begin{columns}
\begin{column}[t]{0.5\columnwidth}
\begin{minted}[]{scala}
object Hero {
...
\begin{frame}[label={sec:org06e458f},fragile]{Summoning the summoner}
 \begin{columns}
\begin{column}[t]{0.5\columnwidth}
\begin{minted}[]{scala}
trait Introducible[A] {
...
\begin{frame}[label={sec:org24e636b}]{What have we done?}
...
\begin{frame}[label={sec:org64be5b4},fragile]{Type classes: abstraction}
 Abstraction holds. Proof is the \texttt{introduce} function itself.
...
\begin{frame}[label={sec:orgc5cd55b},fragile]{Type classes: composition}
...
\begin{frame}[label={sec:org0c51485}]{Type classes: extensibility}
...
\begin{frame}[label={sec:org38c12a6}]{Type classes: final thoughts}
\begin{enumerate}
...
\section*{Transition}
\label{sec:org9f27e44}

\begin{frame}[label={sec:org8210e55}]{Warning}
...
\section*{Semigroup}
\label{sec:orgf214954}

\begin{frame}[label={sec:org529d053}]{Semigroup}
...
\begin{frame}[label={sec:org4e63537},fragile]{But it’s not that scary}
 \begin{columns}
...
\begin{frame}[label={sec:org2d53c0a}]{Important!}
...
\begin{frame}[label={sec:org18e0f27},fragile]{What is law?}
 \begin{itemize}
...
\begin{frame}[label={sec:org8a3fbfb},fragile]{Instance example}
 \begin{minted}[]{scala}
...
\begin{frame}[label={sec:org2719450},fragile]{Checking laws - \sout{pen and paper} in comments}
 \begin{minted}[]{scala}
package object implicits {
  implicit val stringSemigroup: Semigroup[String] = new Semigroup[String] {
    override def combine(x: String, y: String): String = x + y
...
\begin{frame}[label={sec:orgc86b3e3}]{You're programmer after all}
\begin{center}
\includegraphics[height=7cm]{images/you-re-programmer.jpg}
\end{center}
\end{frame}

\begin{frame}[label={sec:org7b83149},fragile]{Question on the interview: property based testing}
 \begin{minted}[]{scala}
...
\begin{frame}[label={sec:org6b44a46},fragile]{More than one valid instance}
 \begin{minted}[]{scala}
package object implicits {
  implicit val stringSemigroup: Semigroup[String] = new Semigroup[String] {
...
\begin{frame}[label={sec:org24f5e8b}]{More examples}
\begin{itemize}
...
\begin{frame}[label={sec:orgaa99b27}]{Contra-examples}
\begin{itemize}
...
\begin{frame}[label={sec:org126e7b8},fragile]{Coding time}
 \begin{enumerate}
...
\section*{Monoid}
\label{sec:org7186b40}

\begin{frame}[label={sec:org421d6d4}]{Monoid}
...
\begin{frame}[label={sec:orgb3172c9},fragile]{Again, it's not that scary}
 \begin{minted}[]{scala}
package object typeclass {
...
\begin{frame}[label={sec:org38408e1},fragile]{Examples}
 \begin{itemize}
...
\label{sec:org2fc5375}

\begin{frame}[label={sec:org1d35eca}]{The most important question}
\pause
...
\section*{Fibonacci}
\label{sec:org99d840c}

\begin{frame}[label={sec:org49586cf}]{The Fibonacci numbers}
\pause
...
\begin{frame}[label={sec:org6697069},fragile]{Solution}
 \begin{columns}
\begin{column}[t]{0.5\columnwidth}
...
\begin{frame}[label={sec:orgdf476ed},fragile]{Two folds}
 \begin{itemize}
...
\begin{frame}[label={sec:org8b0e055},fragile]{Two folds}
 \begin{itemize}
\item \texttt{def foldl[A, B](xs: Seq[A])(z: B)(op: B => A => B): B}
...
\begin{frame}[label={sec:orgfad5b3e},fragile]{Two folds}
 \begin{itemize}
\item \texttt{def foldl[A, B](xs: Seq[A])(z: B)(op: B => A => B): B}
\item \texttt{def foldr[A, B](xs: Seq[A])(z: B)(op: A => B => B): B}
...
\begin{frame}[label={sec:org3e7f137}]{Two folds}
\begin{columns}
\begin{column}[t]{0.5\columnwidth}
\begin{block}{foldl}
\begin{center}
...
\begin{frame}[label={sec:org9d4e71a},fragile]{What Monoid gives us}
 \begin{itemize}
...
\begin{frame}[label={sec:org96e3081}]{Power in terms of Monoid}
...
\begin{frame}[label={sec:org510b992}]{Power in terms of Monoid}
\begin{center}
...
\begin{frame}[label={sec:org0d62d83}]{Power in terms of Monoid}
\begin{center}
...
\begin{frame}[label={sec:org28da97a},fragile]{Power in terms of Monoid}
...
\begin{frame}[label={sec:org4436a7b}]{Back to Fibonacci}
...
\begin{frame}[label={sec:orgeb60349}]{Back to Fibonacci}
\begin{equation*}
  \begin{pmatrix}
    F_{n+1} & F_n \\
    F_n & F_{n-1}
...
\begin{frame}[label={sec:orgcee64d5},fragile]{Coding time}
 \begin{itemize}
...
\begin{frame}[label={sec:org47cf492}]{Profiling results}
\begin{center}
\begin{tabular}{rrr}
N & Matrix & Tailrec\\
\hline
...
\begin{frame}[label={sec:orgc9e8d59},fragile]{Outcome}
 \begin{itemize}
...
\section*{Books}
\label{sec:org44350dc}

\begin{frame}[label={sec:org9d003dd}]{Folds with Monoids}
\begin{itemize}
...
\begin{frame}[label={sec:orgcb1bf89}]{Folds with Monoids}
\begin{center}
...
\begin{frame}[label={sec:org6ad1737},fragile]{MapReduce}
 \begin{itemize}
...
\begin{frame}[label={sec:org19d1b80},fragile]{Coding time}
 \begin{itemize}
...
\begin{frame}[label={sec:orgd682966},fragile]{Benchmarks}
 \begin{verbatim}
Par
duration = 65633 ms
...
\begin{frame}[label={sec:org3c2865f},fragile]{Outcome}
 \begin{itemize}
...
\begin{frame}[label={sec:org2537d6f},fragile]{Homework}
...
\section*{Logger}
\label{sec:org47e3c35}

\begin{frame}[label={sec:org506855d}]{Many monoids}
...
\begin{frame}[label={sec:org8e79d34},fragile]{Let's start with some wrappers (pun intended)}
 \begin{itemize}
...
\begin{frame}[label={sec:org746b8e2},fragile]{Wrappers of monoids are monoids}
 \begin{itemize}
...
\begin{frame}[label={sec:org1983dd4},fragile]{Logger}
 \begin{itemize}
...
\begin{frame}[label={sec:orge37a03c},fragile]{Logger}
 \begin{minted}[]{scala}
...
\begin{frame}[label={sec:orge47c250},fragile]{Logger}
 \begin{itemize}
...
\begin{frame}[label={sec:org4519821},fragile]{Bonus questions}
 \begin{itemize}
...
\begin{frame}[label={sec:orgf639f7a},fragile]{Outcome}
 \begin{itemize}
...
\section*{Recap}
\label{sec:org684857b}

\begin{frame}[label={sec:org2c15506},fragile]{Recap (recup?)}
 \begin{itemize}
...
\begin{frame}[label={sec:orgdab03aa}]{Questions?}
\centerline{\huge $\epsilon \rho \omega \tau \eta \sigma \eta$?}
\end{frame}

\begin{frame}[label={sec:orgfdd809d}]{Thank you very much!}
