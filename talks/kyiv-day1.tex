...
\begin{frame}[label={sec:org8927eab}]{Plan}
\begin{itemize}
\item Type classes
...
\section*{Typeclasses}
\label{sec:org6f1ef7e}

\begin{frame}[label={sec:org12fc6c7}]{Fulfilling a dream}
\begin{itemize}
...
\begin{frame}[label={sec:orgd4688ee},fragile]{Meet the hero}
 \begin{minted}[]{scala}
...
\begin{frame}[label={sec:org2a5d16c},fragile]{Introducing abstractions}
 \begin{minted}[]{scala}
trait Introducible {
  def introduce(): String
}
...
\begin{frame}[label={sec:orgefa714b},fragile]{Using the \texttt{trait}}
 \begin{minted}[]{scala}
object Game extends App {
  val player = Hero("Valik", "Black Mage", 20)
  val orc = Orc("Garrosh", 105)
...
\begin{frame}[label={sec:orgc5a9525},fragile]{Here comes the cockatrice}
 \begin{minted}[]{scala}
...
\begin{frame}[label={sec:orgf227dc7},fragile]{Shawarma to the rescue}
 \begin{columns}
...
\begin{frame}[label={sec:orga7e0392}]{Properties we care about}
\begin{itemize}
...
\begin{frame}[label={sec:orgdd7346a}]{Example}
\begin{columns}
\begin{column}[t]{0.5\columnwidth}
\begin{block}{foldl}
\begin{center}
...
\begin{frame}[label={sec:org3fcdc0e}]{Back to Fibonacci}
...
\begin{frame}[label={sec:orgd15cf5d},fragile]{Time for work}
 \begin{itemize}
...
\begin{frame}[label={sec:org970da8a}]{Outcome}
\begin{itemize}
...
\begin{frame}[label={sec:orgce7085a},fragile]{MapReduce}
 \begin{itemize}
...
\begin{frame}[label={sec:orgd680c90},fragile]{Getting the top used words from set of books}
 \begin{itemize}
...
\section*{Logger}
\label{sec:org0387304}

\begin{frame}[label={sec:org4ef1811},fragile]{Things to note}
 \begin{itemize}
...
\begin{frame}[label={sec:orgd464011},fragile]{Let's start with some wrappers (pun intended)}
 \begin{itemize}
...
\begin{frame}[label={sec:org129ab84},fragile]{Wrappers of monoids are monoids}
 \begin{itemize}
...
\begin{frame}[label={sec:org61fbb94},fragile]{Logger}
 \begin{itemize}
...
\begin{frame}[label={sec:org62737a8},fragile]{Logger}
 \begin{itemize}
...
\section*{Recap}
\label{sec:org6e7d391}

\begin{frame}[label={sec:orgf774200},fragile]{Recap (recup?)}
 \begin{itemize}
...
\begin{frame}[label={sec:orgf6f5737}]{Questions?}
\centerline{\huge $\epsilon \rho \omega \tau \eta \sigma \eta$?}
\end{frame}

\begin{frame}[label={sec:orgcdef3c1}]{Thank you very much!}
