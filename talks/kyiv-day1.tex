...
\section*{Intro}
\label{sec:orgd4709b3}

\begin{frame}[label={sec:org77b0fe5}]{About the Forkshop}
\begin{itemize}
...
\begin{frame}[label={sec:orgb6629c1},fragile]{Whys}
 \begin{itemize}
...
\section*{Today}
\label{sec:org12d6165}

\begin{frame}[label={sec:org4b3a716}]{Agenda}
\begin{itemize}
\item Type classes
\item Semigroups
\item Monoids
...
\label{sec:orgd44a474}

\begin{frame}[label={sec:orgaf30dd1}]{Application definition}
\begin{itemize}
...
\begin{frame}[label={sec:org4f43d2c},fragile]{Meet the hero}
 \begin{minted}[]{scala}
...
\begin{frame}[label={sec:org911472c},fragile]{Every hero needs a monster}
 \begin{minted}[]{scala}
...
\begin{frame}[label={sec:org84d2726},fragile]{Game}
 \begin{columns}
...
\begin{frame}[label={sec:org539892c},fragile]{Introducing abstractions}
 \begin{columns}
...
\begin{frame}[label={sec:org49bc2da},fragile]{Game with trait}
 \begin{columns}
\begin{column}[t]{0.5\columnwidth}
\begin{minted}[]{scala}
...
\begin{frame}[label={sec:orgb99b24f},fragile]{Here comes the cockatrice}
 \begin{columns}
...
\begin{frame}[label={sec:org607415a},fragile]{Shawarma to the rescue}
 \begin{columns}
...
\begin{frame}[label={sec:org8daa9c8}]{Calm down and reevaluate our goal}
\begin{itemize}
...
\begin{frame}[label={sec:orgff9cb0f},fragile]{\texttt{trait} + wrapper: abstraction}
...
\begin{frame}[label={sec:org99109f3},fragile]{\texttt{trait} + wrapper: composition}
...
\begin{frame}[label={sec:orgadfc69f},fragile]{\texttt{trait} + wrapper: extensibility}
...
\begin{frame}[label={sec:org1e96c47}]{You know where it’s going to, right?}
\pause
...
\begin{frame}[label={sec:orga1537d9},fragile]{Dividing data and behaviour}
 \begin{columns}
\begin{column}[t]{0.5\columnwidth}
\begin{minted}[]{scala}
trait Introducible {
...
\begin{frame}[label={sec:org802650f},fragile]{Usage}
 \begin{minted}[]{scala}
// Define new trait
trait Introducible[A] {
  def introduce(a: A): String
...
\begin{frame}[label={sec:org7d53b5d},fragile]{External types? Pff\ldots{}}
 \begin{minted}[]{scala}
import io.proprietary.monsters.cockatrice._
...
\begin{frame}[label={sec:org5b85581}]{But passing implementation around is\ldots{}}
\begin{center}
...
\begin{frame}[label={sec:orgcd11847},fragile]{So implicits :(}
 \begin{columns}
\begin{column}[t]{0.5\columnwidth}
\begin{minted}[]{scala}
object Hero {
...
\begin{frame}[label={sec:org0827d18},fragile]{Summoning the summoner}
 \begin{columns}
\begin{column}[t]{0.5\columnwidth}
\begin{minted}[]{scala}
trait Introducible[A] {
...
\begin{frame}[label={sec:orgce2b431}]{What have we done?}
...
\begin{frame}[label={sec:org5075794},fragile]{Type classes: abstraction}
 Abstraction holds. Proof is the \texttt{introduce} function itself.
...
\begin{frame}[label={sec:org1c4328b},fragile]{Type classes: composition}
...
\begin{frame}[label={sec:org4aa4a62}]{Type classes: extensibility}
...
\begin{frame}[label={sec:orgee0d190}]{Type classes: final thoughts}
\begin{enumerate}
...
\section*{Semigroup}
\label{sec:org6454976}

\begin{frame}[label={sec:orge0f41a8}]{Time for a quiz!}
\begin{columns}
\begin{column}[t]{0.5\columnwidth}
...
\begin{frame}[label={sec:org66920c8}]{Associativity}
\begin{enumerate}
...
\begin{frame}[label={sec:org189e54b}]{Semigroup}
...
\begin{frame}[label={sec:org56745df},fragile]{But it’s not that scary}
 \begin{columns}
...
\begin{frame}[label={sec:orge7efa47}]{Important!}
...
\begin{frame}[label={sec:org28fe104},fragile]{What is law?}
 \begin{itemize}
...
\begin{frame}[label={sec:orge7a870d},fragile]{Instance example}
 \begin{minted}[]{scala}
...
\begin{frame}[label={sec:org7b83e46},fragile]{Checking laws - \sout{pen and paper} in comments}
 \begin{minted}[]{scala}
package object implicits {
  implicit val stringSemigroup: Semigroup[String] = new Semigroup[String] {
    override def combine(x: String, y: String): String = x + y
...
\begin{frame}[label={sec:org5974be3}]{You're programmer after all}
\begin{center}
\includegraphics[height=7cm]{images/you-re-programmer.jpg}
\end{center}
\end{frame}

\begin{frame}[label={sec:orgbc54ff1},fragile]{Question on the interview: property based testing}
 \begin{minted}[]{scala}
...
\begin{frame}[label={sec:orgbad0958},fragile]{More than one valid instance}
 \begin{minted}[]{scala}
package object implicits {
  implicit val stringSemigroup: Semigroup[String] = new Semigroup[String] {
...
\begin{frame}[label={sec:org2c1b271}]{More examples}
\begin{itemize}
...
\begin{frame}[label={sec:org62dcbb0}]{Contra-examples}
\begin{itemize}
...
\begin{frame}[label={sec:org414b704},fragile]{Coding time}
 \begin{enumerate}
...
\section*{Monoid}
\label{sec:org12a3233}

\begin{frame}[label={sec:org395a352}]{Monoid}
\begin{itemize}
...
\begin{frame}[label={sec:org11dbd49}]{Monoid}
...
\begin{frame}[label={sec:org4640cee}]{Monoid}
Back to fancy words.
...
\begin{frame}[label={sec:orgda88485},fragile]{Again, it's not that scary}
 \begin{minted}[]{scala}
package object typeclass {
...
\begin{frame}[label={sec:orge1b7001},fragile]{Examples}
 \begin{itemize}
...
\begin{frame}[label={sec:orgb50967c}]{The most important question}
\pause
...
\section*{Fibonacci}
\label{sec:orgc829bf1}

\begin{frame}[label={sec:orgebc8797}]{The Fibonacci numbers}
\pause
...
\begin{frame}[label={sec:orge128ce2},fragile]{Solution}
 \begin{columns}
\begin{column}[t]{0.5\columnwidth}
...
\begin{frame}[label={sec:org4b43ff3},fragile]{Two folds (mod)}
 \begin{itemize}
...
\begin{frame}[label={sec:org3a8a466},fragile]{Two folds}
 \begin{itemize}
\item \texttt{def foldl[A, B](xs: Seq[A])(z: B)(op: B => A => B): B}
...
\begin{frame}[label={sec:org8aa750c},fragile]{Two folds}
 \begin{itemize}
\item \texttt{def foldl[A, B](xs: Seq[A])(z: B)(op: B => A => B): B}
\item \texttt{def foldr[A, B](xs: Seq[A])(z: B)(op: A => B => B): B}
...
\begin{frame}[label={sec:org392151f}]{Two folds}
\begin{columns}
\begin{column}[t]{0.5\columnwidth}
\begin{block}{foldl}
$$+ : B \rightarrow A \rightarrow B$$
...
\begin{frame}[label={sec:orgab00ae7},fragile]{What Monoid gives us}
 \begin{itemize}
...
\begin{frame}[label={sec:orge8fb265}]{Power in terms of Monoid}
...
\begin{frame}[label={sec:org3b6bdb4}]{Power in terms of Monoid}
\begin{center}
...
\begin{frame}[label={sec:orgc205c51}]{Power in terms of Monoid}
\begin{center}
...
\begin{frame}[label={sec:org0b95445},fragile]{Power in terms of Monoid}
...
\begin{frame}[label={sec:org600c10f}]{Back to Fibonacci}
...
\begin{frame}[label={sec:orgb95b1a8}]{Back to Fibonacci}
\begin{equation*}
  \begin{pmatrix}
    F_{n+1} & F_n \\
    F_n & F_{n-1}
...
\begin{frame}[label={sec:org0cea091},fragile]{Coding time}
 \begin{itemize}
...
\begin{frame}[label={sec:org8a58604}]{Profiling results}
\begin{center}
\begin{tabular}{rrr}
N & Matrix & Tailrec\\
\hline
...
\begin{frame}[label={sec:org90d2917},fragile]{Outcome}
 \begin{itemize}
...
\section*{Books}
\label{sec:org5378a97}

\begin{frame}[label={sec:orgab0d8cf}]{Folds with Monoids}
\begin{itemize}
...
\begin{frame}[label={sec:org4a71f4f}]{Folds with Monoids}
\begin{center}
...
\begin{frame}[label={sec:org29d681f},fragile]{MapReduce}
 \begin{itemize}
...
\begin{frame}[label={sec:orgaa1c210},fragile]{Coding time}
 \begin{itemize}
...
\begin{frame}[label={sec:org78e94e7},fragile]{Benchmarks}
 \begin{verbatim}
Par
duration = 65633 ms
...
\begin{frame}[label={sec:orgdfd2f04},fragile]{Outcome}
 \begin{itemize}
...
\begin{frame}[label={sec:org804afe7},fragile]{Homework}
...
\begin{frame}[label={sec:orgb8d9127}]{Many monoids}
...
\begin{frame}[label={sec:orgc63d620},fragile]{Let's start with some wrappers (pun intended)}
 \begin{itemize}
...
\begin{frame}[label={sec:org5a865a8},fragile]{Wrappers of monoids are monoids}
 \begin{itemize}
...
\begin{frame}[label={sec:org08a2520},fragile]{Logger}
 \begin{itemize}
...
\begin{frame}[label={sec:orgbf42304},fragile]{Logger}
 \begin{minted}[]{scala}
...
\begin{frame}[label={sec:org349119a},fragile]{Logger}
 \begin{itemize}
...
\begin{frame}[label={sec:orgca5fe94},fragile]{Bonus questions}
 \begin{itemize}
...
\begin{frame}[label={sec:orge9c3ed5},fragile]{Outcome}
 \begin{itemize}
...
\section*{Recap}
\label{sec:org8d41c1e}

\begin{frame}[label={sec:org22491f0},fragile]{Recap (recup?)}
 \begin{itemize}
...
\begin{frame}[label={sec:org3184aca}]{Questions?}
\centerline{\huge $\epsilon \rho \omega \tau \eta \sigma \eta \varsigma$?}
\end{frame}

\begin{frame}[label={sec:org4c99d9e}]{Thank you very much!}
