...
\section*{Intro}
\label{sec:org64e3249}

\begin{frame}[label={sec:org480f276}]{About us}
\begin{block}{Valik}
...
\begin{frame}[label={sec:orgf751342}]{About the Forkshop}
\begin{itemize}
...
\begin{frame}[label={sec:org1e3ffa1},fragile]{Whys}
 \begin{itemize}
...
\section*{Today}
\label{sec:org785bde8}

\begin{frame}[label={sec:org8d20c5f}]{Agenda}
\begin{itemize}
\item Type classes
\item Semigroups
\item Monoids
...
\label{sec:org01d1d8c}

\begin{frame}[label={sec:org29f5a6a}]{Type classes}
\begin{itemize}
\item <1-> Problem:
\begin{itemize}
...
\begin{frame}[label={sec:orga688a88}]{Application definition}
\begin{itemize}
...
\begin{frame}[label={sec:org3818f33},fragile]{Meet the hero}
 \begin{minted}[]{scala}
...
\begin{frame}[label={sec:org59a1ce2},fragile]{Every hero needs a monster}
 \begin{minted}[]{scala}
...
\begin{frame}[label={sec:orgb8a3a60},fragile]{Game}
 \begin{columns}
...
\begin{frame}[label={sec:orge7c2245},fragile]{Be DRY, leap for disaster}
 \begin{columns}
\begin{column}[t]{0.46\columnwidth}
\begin{minted}[]{scala}
object Game extends App {
...
\begin{frame}[label={sec:orgdf874c0},fragile]{Introducing abstractions}
 \begin{columns}
...
\begin{frame}[label={sec:orgcf7f4f4},fragile]{Game with \texttt{trait}}
 \begin{columns}
\begin{column}[t]{0.5\columnwidth}
\begin{minted}[]{scala}
def introduce(phrase: String): Unit = {
...
\begin{frame}[label={sec:orgbe52316},fragile]{Game with trait}
 \begin{itemize}
...
\begin{frame}[label={sec:org6ff30ca},fragile]{Here comes the cockatrice}
 \begin{minted}[]{scala}
...
\begin{frame}[label={sec:org065b194},fragile]{Shawarma to the rescue}
 \begin{columns}
...
\begin{frame}[label={sec:org41ec624}]{Calm down and reevaluate our goal}
\begin{itemize}
...
\begin{frame}[label={sec:orgda6f561},fragile]{\texttt{trait} + wrapper: abstraction}
...
\begin{frame}[label={sec:org9c968b9},fragile]{\texttt{trait} + wrapper: composition}
...
\begin{frame}[label={sec:org2f6ad9c},fragile]{\texttt{trait} + wrapper: extensibility}
...
\begin{frame}[label={sec:org9d43785}]{You know where it’s going to, right?}
\pause
...
\begin{frame}[label={sec:org951a597},fragile]{Dividing data and behaviour}
 \begin{columns}
\begin{column}[t]{0.5\columnwidth}
\begin{minted}[]{scala}
trait Introducible {
...
\begin{frame}[label={sec:org51ec5c6},fragile]{Usage}
 \begin{minted}[]{scala}
// Define new trait
trait Introducible[A] {
  def introduce(a: A): String
...
\begin{frame}[label={sec:org0982387},fragile]{External types? Pff\ldots{}}
 \begin{minted}[]{scala}
import io.proprietary.monsters.cockatrice._
...
\begin{frame}[label={sec:org4d6f3fd}]{But passing implementation around is\ldots{}}
\begin{center}
...
\begin{frame}[label={sec:org9b4d226},fragile]{So implicits :(}
 \begin{columns}
\begin{column}[t]{0.5\columnwidth}
\begin{minted}[]{scala}
object Hero {
...
\begin{frame}[label={sec:orgf80bb2c},fragile]{Summoning the summoner}
 \begin{columns}
\begin{column}[t]{0.5\columnwidth}
\begin{minted}[]{scala}
trait Introducible[A] {
...
\begin{frame}[label={sec:org25569c2}]{What have we done?}
...
\begin{frame}[label={sec:org23edb80},fragile]{Type classes: abstraction}
 Abstraction holds. Proof is the \texttt{introduce} function itself.
...
\begin{frame}[label={sec:org365d1ec},fragile]{Type classes: composition}
...
\begin{frame}[label={sec:orgf09ff01}]{Type classes: extensibility}
...
\begin{frame}[label={sec:orgf21f84d}]{Type classes: final thoughts}
\begin{enumerate}
...
\section*{Transition}
\label{sec:orge08bbeb}

\begin{frame}[label={sec:org0e13711}]{Warning}
...
\section*{Semigroup}
\label{sec:org121599a}

\begin{frame}[label={sec:orge154921}]{Semigroup}
...
\begin{frame}[label={sec:org195f837},fragile]{But it’s not that scary}
 \begin{columns}
...
\begin{frame}[label={sec:orga4936fc}]{Important!}
...
\begin{frame}[label={sec:orgfe9a936},fragile]{What is law?}
 \begin{itemize}
...
\begin{frame}[label={sec:orge2fd705},fragile]{Instance example}
 \begin{minted}[]{scala}
...
\begin{frame}[label={sec:org2a35b66},fragile]{Checking laws - \sout{pen and paper} in comments}
 \begin{minted}[]{scala}
package object implicits {
  implicit val stringSemigroup: Semigroup[String] = new Semigroup[String] {
    override def combine(x: String, y: String): String = x + y
...
\begin{frame}[label={sec:org0d3b4e4}]{You're programmer after all}
\begin{center}
\includegraphics[height=7cm]{images/you-re-programmer.jpg}
\end{center}
\end{frame}

\begin{frame}[label={sec:org7b4cb93},fragile]{Question on the interview: property based testing}
 \begin{minted}[]{scala}
...
\begin{frame}[label={sec:org555d99a},fragile]{More than one valid instance}
 \begin{minted}[]{scala}
package object implicits {
  implicit val stringSemigroup: Semigroup[String] = new Semigroup[String] {
...
\begin{frame}[label={sec:orgde6286d}]{More examples}
\begin{itemize}
...
\begin{frame}[label={sec:orgaa61748}]{Contra-examples}
\begin{itemize}
...
\begin{frame}[label={sec:orgb13cbda},fragile]{Coding time}
 \begin{enumerate}
...
\section*{Monoid}
\label{sec:org6e8bcee}

\begin{frame}[label={sec:org08368ec}]{Monoid}
...
\begin{frame}[label={sec:org7dc9ff8},fragile]{Again, it's not that scary}
 \begin{minted}[]{scala}
package object typeclass {
...
\begin{frame}[label={sec:org7bfd911},fragile]{Examples}
 \begin{itemize}
...
\label{sec:org74a2720}

\begin{frame}[label={sec:org2f7a719}]{The most important question}
\pause
...
\section*{Fibonacci}
\label{sec:org95745ac}

\begin{frame}[label={sec:org757da97}]{The Fibonacci numbers}
\pause
...
\begin{frame}[label={sec:org3ad4508},fragile]{Solution}
 \begin{columns}
\begin{column}[t]{0.5\columnwidth}
...
\begin{frame}[label={sec:orgaa303fd},fragile]{Two folds}
 \begin{itemize}
...
\begin{frame}[label={sec:orgdb927da},fragile]{Two folds}
 \begin{itemize}
\item \texttt{def foldl[A, B](xs: Seq[A])(z: B)(op: B => A => B): B}
...
\begin{frame}[label={sec:org04ac196},fragile]{Two folds}
 \begin{itemize}
\item \texttt{def foldl[A, B](xs: Seq[A])(z: B)(op: B => A => B): B}
\item \texttt{def foldr[A, B](xs: Seq[A])(z: B)(op: A => B => B): B}
...
\begin{frame}[label={sec:orgd7b4ff9}]{Two folds}
\begin{columns}
\begin{column}[t]{0.5\columnwidth}
\begin{block}{foldl}
\begin{center}
...
\begin{frame}[label={sec:org40c3d16},fragile]{What Monoid gives us}
 \begin{itemize}
...
\begin{frame}[label={sec:org52aeb2b}]{Power in terms of Monoid}
...
\begin{frame}[label={sec:org89d7c35}]{Power in terms of Monoid}
\begin{center}
...
\begin{frame}[label={sec:orge56a32f}]{Power in terms of Monoid}
\begin{center}
...
\begin{frame}[label={sec:org40a772e},fragile]{Power in terms of Monoid}
...
\begin{frame}[label={sec:orge8ed986}]{Back to Fibonacci}
...
\begin{frame}[label={sec:org154c288}]{Back to Fibonacci}
\begin{equation*}
  \begin{pmatrix}
    F_{n+1} & F_n \\
    F_n & F_{n-1}
...
\begin{frame}[label={sec:org860c1a0},fragile]{Coding time}
 \begin{itemize}
...
\begin{frame}[label={sec:org48d1d15}]{Profiling results}
\begin{center}
\begin{tabular}{rrr}
N & Matrix & Tailrec\\
\hline
...
\begin{frame}[label={sec:org0877d20},fragile]{Outcome}
 \begin{itemize}
...
\section*{Books}
\label{sec:org3964d51}

\begin{frame}[label={sec:orgc772731}]{Folds with Monoids}
\begin{itemize}
...
\begin{frame}[label={sec:org2f77b75}]{Folds with Monoids}
\begin{center}
...
\begin{frame}[label={sec:org3270db9},fragile]{MapReduce}
 \begin{itemize}
...
\begin{frame}[label={sec:org81456e6},fragile]{Coding time}
 \begin{itemize}
...
\begin{frame}[label={sec:orge67d6e7},fragile]{Benchmarks}
 \begin{verbatim}
Par
duration = 65633 ms
...
\begin{frame}[label={sec:org3d78461},fragile]{Outcome}
 \begin{itemize}
...
\begin{frame}[label={sec:orgd25cfff},fragile]{Homework}
...
\section*{Logger}
\label{sec:orgc70bdda}

\begin{frame}[label={sec:org0581cd3}]{Many monoids}
...
\begin{frame}[label={sec:org0ece765},fragile]{Let's start with some wrappers (pun intended)}
 \begin{itemize}
...
\begin{frame}[label={sec:orge3ede0d},fragile]{Wrappers of monoids are monoids}
 \begin{itemize}
...
\begin{frame}[label={sec:org50d2c27},fragile]{Logger}
 \begin{itemize}
...
\begin{frame}[label={sec:org33f8b84},fragile]{Logger}
 \begin{minted}[]{scala}
...
\begin{frame}[label={sec:org472307a},fragile]{Logger}
 \begin{itemize}
...
\begin{frame}[label={sec:orgf7f98eb},fragile]{Bonus questions}
 \begin{itemize}
...
\begin{frame}[label={sec:orgffc19cc},fragile]{Outcome}
 \begin{itemize}
\item <1-> If you have a monoid, it's easy to form new monoid (of a higher rank).
\item <2-> Functions can also be monoid. This is really cool by itself.
\item <3-> Some of you probably gonna write a new \texttt{colog} lib (but for Scala).
\end{itemize}
\end{frame}

\section*{Recap}
\label{sec:org293c8d0}

\begin{frame}[label={sec:org5abc290},fragile]{Recap (recup?)}
 \begin{itemize}
...
\begin{frame}[label={sec:org3989b07}]{Questions?}
\centerline{\huge $\epsilon \rho \omega \tau \eta \sigma \eta$?}
\end{frame}

\begin{frame}[label={sec:org3334d6f}]{Thank you very much!}
