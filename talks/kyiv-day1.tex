...
\section*{Intro}
\label{sec:orgd31672f}

\begin{frame}[label={sec:org89112b3}]{About us}
\begin{block}{Valik}
...
\begin{frame}[label={sec:orgeae8812}]{About the Forkshop}
\begin{itemize}
...
\begin{frame}[label={sec:org3e9c8a9},fragile]{Whys}
 \begin{itemize}
...
\section*{Today}
\label{sec:orge6323d6}

\begin{frame}[label={sec:orgb5dd3aa}]{Agenda}
\begin{itemize}
\item Type classes
\item Semigroups
\item Monoids
...
\label{sec:org8150a52}

\begin{frame}[label={sec:orgb920f11}]{Type classes}
\begin{itemize}
\item Problem:
\begin{itemize}
...
\begin{frame}[label={sec:org0ba421d}]{Application definition}
\begin{itemize}
...
\begin{frame}[label={sec:org7591c7a},fragile]{Meet the hero}
 \begin{minted}[]{scala}
...
\begin{frame}[label={sec:orgf3f1b2c},fragile]{Every hero needs a monster}
 \begin{minted}[]{scala}
...
\begin{frame}[label={sec:org0fda5b5},fragile]{Game}
 \begin{columns}
...
\begin{frame}[label={sec:org6b495f5},fragile]{Be DRY, leap for disaster}
 \begin{columns}
\begin{column}[t]{0.46\columnwidth}
\begin{minted}[]{scala}
object Game extends App {
...
\begin{frame}[label={sec:orge568ddf},fragile]{Introducing abstractions}
 \begin{columns}
...
\begin{frame}[label={sec:orgc8982cf},fragile]{Game with \texttt{trait}}
 \begin{columns}
\begin{column}[t]{0.5\columnwidth}
\begin{minted}[]{scala}
def introduce(phrase: String): Unit = {
...
\begin{frame}[label={sec:orgc16a71e},fragile]{Game with trait}
 \begin{itemize}
...
\begin{frame}[label={sec:org8b75d20},fragile]{Here comes the cockatrice}
 \begin{minted}[]{scala}
...
\begin{frame}[label={sec:orgc3143e0},fragile]{Shawarma to the rescue}
 \begin{columns}
...
\begin{frame}[label={sec:org016a3aa}]{Calm down and reevaluate our goal}
\begin{itemize}
...
\begin{frame}[label={sec:org55300ea},fragile]{\texttt{trait} + wrapper: abstraction}
...
\begin{frame}[label={sec:org8c1c9b8},fragile]{\texttt{trait} + wrapper: composition}
...
\begin{frame}[label={sec:org1f26005},fragile]{\texttt{trait} + wrapper: extensibility}
...
\begin{frame}[label={sec:org1c9a4aa}]{You know where it’s going to, right?}
\pause
...
\begin{frame}[label={sec:org6c94af1},fragile]{Dividing data and behaviour}
 \begin{columns}
\begin{column}[t]{0.5\columnwidth}
\begin{minted}[]{scala}
trait Introducible {
...
\begin{frame}[label={sec:org7407ca3},fragile]{Usage}
 \begin{minted}[]{scala}
// Define new trait
trait Introducible[A] {
  def introduce(a: A): String
...
\begin{frame}[label={sec:orgb3ca13c},fragile]{External types? Pff\ldots{}}
 \begin{minted}[]{scala}
import io.proprietary.monsters.cockatrice._
...
\begin{frame}[label={sec:org6168a1c}]{But passing implementation around is\ldots{}}
\begin{center}
...
\begin{frame}[label={sec:org2a4709f},fragile]{So implicits :(}
 \begin{columns}
\begin{column}[t]{0.5\columnwidth}
\begin{minted}[]{scala}
object Hero {
...
\begin{frame}[label={sec:orgd257b71},fragile]{Summoning the summoner}
 \begin{columns}
\begin{column}[t]{0.5\columnwidth}
\begin{minted}[]{scala}
trait Introducible[A] {
...
\begin{frame}[label={sec:orgafa4998}]{What have we done?}
...
\begin{frame}[label={sec:org19da20b},fragile]{Type classes: abstraction}
 Abstraction holds. Proof is the \texttt{introduce} function itself.
...
\begin{frame}[label={sec:org09cf8db},fragile]{Type classes: composition}
...
\begin{frame}[label={sec:org61c1a20}]{Type classes: extensibility}
...
\begin{frame}[label={sec:orgb438457}]{Type classes: final thoughts}
\begin{enumerate}
...
\section*{Transition}
\label{sec:orgc0ffd87}

\begin{frame}[label={sec:orgd71a0ce}]{Warning}
...
\section*{Semigroup}
\label{sec:orgafd3b2a}

\begin{frame}[label={sec:org1f19aff}]{Semigroup}
...
\begin{frame}[label={sec:orga6165b2},fragile]{But it’s not that scary}
 \begin{columns}
...
\begin{frame}[label={sec:orgf5ecac9}]{Important!}
...
\begin{frame}[label={sec:orgb5c2f68},fragile]{What is law?}
 \begin{itemize}
...
\begin{frame}[label={sec:org5d957f0},fragile]{Instance example}
 \begin{minted}[]{scala}
...
\begin{frame}[label={sec:org999f7a4},fragile]{Checking laws - \sout{pen and paper} in comments}
 \begin{minted}[]{scala}
package object implicits {
  implicit val stringSemigroup: Semigroup[String] = new Semigroup[String] {
    override def combine(x: String, y: String): String = x + y
...
\begin{frame}[label={sec:org1fcace3}]{You're programmer after all}
\begin{center}
\includegraphics[height=7cm]{images/you-re-programmer.jpg}
\end{center}
\end{frame}

\begin{frame}[label={sec:org27af796},fragile]{Question on the interview: property based testing}
 \begin{minted}[]{scala}
...
\begin{frame}[label={sec:org7fa2811},fragile]{More than one valid instance}
 \begin{minted}[]{scala}
package object implicits {
  implicit val stringSemigroup: Semigroup[String] = new Semigroup[String] {
...
\begin{frame}[label={sec:orgbde98c3}]{More examples}
\begin{itemize}
...
\begin{frame}[label={sec:org9f4c8c9}]{Contra-examples}
\begin{itemize}
...
\begin{frame}[label={sec:org0ce5c5d},fragile]{Lets hack}
 \begin{enumerate}
...
\section*{Monoid}
\label{sec:org1816c25}

\begin{frame}[label={sec:org9d4508a}]{Monoid}
...
\begin{frame}[label={sec:orgb238140},fragile]{Again, it's not that scary}
 \begin{minted}[]{scala}
package object typeclass {
...
\begin{frame}[label={sec:orgfcbb8a8},fragile]{Examples}
 \begin{itemize}
...
\label{sec:orgc4c02e5}

\begin{frame}[label={sec:orgec2299a}]{The most important question}
\begin{center}
...
\section*{Fibonacci}
\label{sec:orgcd09303}

\begin{frame}[label={sec:org802b1f3}]{The Fibonacci numbers}
...
\begin{frame}[label={sec:orgdb8bf0a},fragile]{Solution}
 \begin{columns}
\begin{column}[t]{0.5\columnwidth}
...
\end{frame}
\begin{frame}[label={sec:org82a8d9b},fragile]{Two folds}
 \begin{itemize}
...
\begin{frame}[label={sec:orgc7241e8}]{Example}
\begin{columns}
\begin{column}[t]{0.5\columnwidth}
\begin{block}{foldl}
\begin{center}
...
\begin{frame}[label={sec:org2b76a59},fragile]{What Monoid gives us}
 \begin{itemize}
...
\begin{frame}[label={sec:org73d4fa4}]{Power in terms of Monoid}
\begin{itemize}
...
\begin{frame}[label={sec:orgb1b6a17}]{Power in terms of Monoid}
\begin{center}
...
\begin{frame}[label={sec:org92af4ba}]{Power in terms of Monoid}
\begin{center}
...
\begin{frame}[label={sec:org760b649},fragile]{Power in terms of Monoid}
...
\begin{frame}[label={sec:org4e9e97f}]{Back to Fibonacci}
...
\begin{frame}[label={sec:org0a34779},fragile]{Time for work}
 \begin{itemize}
...
\begin{frame}[label={sec:org38d59c4}]{Outcome}
\begin{itemize}
...
\section*{Books}
\label{sec:org7de168b}

\begin{frame}[label={sec:org1cccb9a}]{Folds with Monoids}
\begin{itemize}
...
\begin{frame}[label={sec:org9c97cc7}]{Folds with Monoids}
\begin{center}
...
\begin{frame}[label={sec:org2a8bf6d},fragile]{MapReduce}
 \begin{itemize}
...
\begin{frame}[label={sec:org494105b},fragile]{Getting the top used words from set of books}
 \begin{itemize}
...
\section*{Logger}
\label{sec:org724a8b2}

\begin{frame}[label={sec:org38d4f9c},fragile]{Things to note}
 \begin{itemize}
...
\begin{frame}[label={sec:org3bb494a},fragile]{Let's start with some wrappers (pun intended)}
 \begin{itemize}
...
\begin{frame}[label={sec:orgb09b82a},fragile]{Wrappers of monoids are monoids}
 \begin{itemize}
...
\begin{frame}[label={sec:orgd987bbf},fragile]{Logger}
 \begin{itemize}
...
\begin{frame}[label={sec:org438adcb},fragile]{Logger}
 \begin{itemize}
...
\section*{Recap}
\label{sec:orga0757c9}

\begin{frame}[label={sec:org489d960},fragile]{Recap (recup?)}
 \begin{itemize}
...
\begin{frame}[label={sec:org2747055}]{Questions?}
\centerline{\huge $\epsilon \rho \omega \tau \eta \sigma \eta$?}
\end{frame}

\begin{frame}[label={sec:orgdd0af19}]{Thank you very much!}
